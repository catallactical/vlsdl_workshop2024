...
class ClassificationPlot(MplPlot):
    ...
    def plot_confusion_matrix(self, **kwargs):
        ...
        if len(self.class_names) > len(conf_mat):  # More classes than expected
            conf_mat = np.pad(
                conf_mat,
                pad_width=[0, len(self.class_names)-conf_mat.shape[0]]
            )
        elif len(self.class_names) < len(conf_mat):  # Handle ignored classes
            known = len(self.class_names)  # Number of known/non-ignored classes
            for i in range(len(self.class_names), len(conf_mat)):
                class_names.append(self.ignore_classes[i-known])
